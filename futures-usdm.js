/* ==========================================================
   futures-usdm.js – Futures > USD-M (top 20 meme coins)
...
const HOLD_KEY='AVX_futures_usdm_holdings_test';
...
async function refreshPrices(){
 try{
  ...
  TOKENS.forEach(([sym,_,id])=>{
   ...
   const el=document.getElementById('price-futures-usdm-'+sym);if(el)el.textContent=fmtINR(p);
  });
 }catch(e){console.error('futures usdm price fetch fail',e);}
}

/* ---------- RENDER TOKEN LIST ---------- */
function renderList(){
 const c=document.getElementById('usdm');if(!c)return;
 c.innerHTML=TOKENS.map(([sym,name])=>`
  <div class="avx-row">
    <div class="avx-left" onclick="AVX_futuresUsdmShowTokenGraph('${sym}')">
      <div class="avx-sym">${sym}</div>
      <div class="avx-name">${name}</div>
      <div class="avx-price" id="price-futures-usdm-${sym}">₹--</div>
    </div>
    <div class="avx-actions">
      <button class="avx-buy" onclick="AVX_futuresUsdmBuyToken('${sym}')">Long</button>
      <button class="avx-sell" onclick="AVX_futuresUsdmSellToken('${sym}')">Short</button>
    </div>
  </div>`).join('');
}
...
function buildTradeModal(){
 const m=document.createElement('div');m.id='avx-futures-usdm-trade-modal';
 m.innerHTML=`
  <div class="avx-t-overlay"></div>
  <div class="avx-t-box">
    <div class="avx-t-head"><span id="avx-t-title">Futures Trade</span><button id="avx-t-close">×</button></div>
...
function showModal({mode,sym,price,bal,holdQty}){
 const m=document.getElementById('avx-futures-usdm-trade-modal')||buildTradeModal();
 ...
 if(mode==='long'){title.textContent=`Long ${sym} Future`;btn.textContent='Open Long';btn.classList.remove('sell');btn.classList.add('buy');}
 else{title.textContent=`Short ${sym} Future`;btn.textContent='Open Short';btn.classList.remove('buy');btn.classList.add('sell');}
...
function hideModal(){const m=document.getElementById('avx-futures-usdm-trade-modal');if(!m)return;m.classList.remove('show');setTimeout(()=>{m.style.display='none';},150);}

/* ---------- CONFIRM TRADE ---------- */
async function confirmTrade(){
 const m=document.getElementById('avx-futures-usdm-trade-modal');if(!m)return;
...
function buildChartModal(){
 const wrap=document.createElement('div');wrap.id='avx-futures-usdm-chart-modal';
 wrap.innerHTML=`
  <div class="avx-c-overlay"></div>
  <div class="avx-c-box">
    <div class="avx-c-head"><span id="avx-c-title">Futures Chart</span><button id="avx-c-close">×</button></div>
    <canvas id="avx-futures-canvas" width="400" height="220"></canvas>
    <div class="avx-c-range-msg">Last 30 days (INR) - Futures</div>
  </div>`;
...
function hideChartModal(){const m=document.getElementById('avx-futures-usdm-chart-modal');if(!m)return;m.classList.remove('show');setTimeout(()=>{m.style.display='none';},150);}
async function showChart(sym){
 const m=document.getElementById('avx-futures-usdm-chart-modal')||buildChartModal();
 m.querySelector('#avx-c-title').textContent=`${sym} Futures Chart`;
 m.style.display='block';requestAnimationFrame(()=>m.classList.add('show'));
 ...
 try{
  ...
  const j=await r.json();const pts=(j.prices||[]).map(p=>+p[1]);drawSimpleLine('avx-futures-canvas',pts);
...
window.AVX_futuresUsdmBuyToken=async(sym)=>{
...
window.AVX_futuresUsdmSellToken=async(sym)=>{
 const p=livePrices[sym]||0;if(p<=0){toast('Price not loaded.',false);return;}
 const bal=await getWalletINR();const hold=await getHolding(sym);
 showModal({mode:'short',sym,price:p,bal,holdQty:hold.qty});
};
window.AVX_futuresUsdmShowTokenGraph=(sym)=>showChart(sym);

/* ---------- CSS INJECTION ---------- */
if(!document.getElementById('avx-futures-usdm-styles')){
 const s=document.createElement('style');s.id='avx-futures-usdm-styles';s.textContent=`
  ...
  #avx-futures-usdm-trade-modal,#avx-futures-usdm-chart-modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;z-index:9999;opacity:0;transition:opacity 0.15s;}
  #avx-futures-usdm-trade-modal.show,#avx-futures-usdm-chart-modal.show{opacity:1;}
