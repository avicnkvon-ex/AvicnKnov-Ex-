/* ==========================================================
   market-crypto.js – Market > Crypto (top ~50 tokens)
...
(function(){
  ...
  /* ---------- TOKEN LIST RENDER ---------- */
  function renderList(){
    const c=document.getElementById('crypto') || document.querySelector('#market #crypto'); if(!c) return;
  ...
  function observeMarketTab(){
    // Listen for Market section becoming visible
        ...
        mutations.forEach(function(mutation) {
          if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            if(marketSection.classList.contains('active')){
              setTimeout(refreshPrices, 500); // Refresh when market section becomes active
            }
          }
        });
      });
    ...
    // Listen for Crypto tab becoming visible
    const cryptoDiv = document.querySelector('#market #crypto');
    if(cryptoDiv){
      const observer2 = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
          if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            if(cryptoDiv.classList.contains('active')){
              setTimeout(refreshPrices, 300); // Refresh when crypto tab becomes active
            }
          }
        });
      });
      observer2.observe(cryptoDiv, { attributes: true });
    }
  }
  ...
  async function init(){
    ...
    // Initial price refresh with delay for DOM readiness
    setTimeout(async ()=>{
      await refreshPrices();
  ...
  if(document.readyState === 'loading'){
    ...
    setTimeout(init, 500); // Small delay if already loaded
  }
